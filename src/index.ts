...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		ctx.waitUntil(this.handleEvent(event, payload, octokit, repoConfig));

		return new Response('Webhook event received and is being processed.', { status: 202 });
	},

	async handleEvent(event: string, payload: any, octokit: Octokit, config: RepoConfig) {
		...
		if (prNumber !== undefined) {
			await this.processPullRequest(repo, prNumber, octokit, config);
		}
	},

	async processPullRequest(repo: any, prNumber: number, octokit: Octokit, config: RepoConfig) {
		...
		try {
			...
			const geminiComments = comments.filter(comment => comment.body?.includes('Gemini Code Assist'));
			console.log(`[${repo.full_name}] Found ${geminiComments.length} Gemini Code Assist comments on PR #${prNumber}.`);
			...
			for (const comment of geminiComments) {
				...
				if (suggestions.length > 0) {
					...
					await this.applySuggestions(repo, pr, suggestions, octokit);
				}
			}
		} catch (error) {
	...
	async applySuggestions(repo: any, pr: any, suggestions: CodeSuggestion[], octokit: Octokit) {
		const owner = repo.owner.login;
		const repoName = repo.name;
		...
		for (const suggestion of suggestions) {
			try {
				...
				const originalContent = Buffer.from(file.content, 'base64').toString('utf-8');
		...
		try {
			...
			const { data: newCommit } = await octokit.rest.git.createCommit({
				owner,
				repo: repoName,
				message: 'feat: Apply Gemini Code Assist suggestions',
				tree: newTree.sha,
				parents: [headSha],
				author: {
					name: 'GH-Worker-Bot',
			...
			// Wait a moment for GitHub to process the new commit, then merge.
			await new Promise(resolve => setTimeout(resolve, 5000)); // 5 second delay
			await this.mergePullRequest(repo, pr.number, octokit);
	...
	async mergePullRequest(repo: any, prNumber: number, octokit: Octokit) {
		const owner = repo.owner.login;
		const repoName = repo.name;
		...
		try {
			...
			if (pr.mergeable === false) {
				console.error(`[${repo.full_name}] PR #${prNumber} is not mergeable. Merge conflicts likely exist.`);
			...
			// Now, integrate into the main branch if necessary
			const targetBranch = pr.base.ref;
			if (targetBranch !== 'main') {
				console.log(`[${repo.full_name}] Integrating changes from '${targetBranch}' into 'main'.`);
				try {
					await octokit.rest.repos.merge({
						...
						base: 'main',
						head: targetBranch,
						commit_message: `Merge branch '${targetBranch}' into main`,
					});
					console.log(`[${repo.full_name}] Successfully merged '${targetBranch}' into 'main'.`);
				} catch (integrationError: any) {
					console.error(`[${repo.full_name}] Failed to merge '${targetBranch}' into 'main':`, integrationError.message);
